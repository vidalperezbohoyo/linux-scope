...
void VideoProcessor::applyCrosshair(cv::Mat& frame)
{
    ...
    cv::line(frame, cv::Point(0, cy), cv::Point(frame.cols, cy), cv::Scalar(0, 255, 0), 1);
    cv::line(frame, cv::Point(cx, 0), cv::Point(cx, frame.rows), cv::Scalar(0, 255, 0), 1);
}

void VideoProcessor::applyRoundFilter(cv::Mat& frame)
{
    if (frame.empty()) return;

    cv::Mat mask = cv::Mat::zeros(frame.size(), frame.type());
    cv::circle(mask, cv::Point(frame.cols / 2, frame.rows / 2), std::min(frame.cols, frame.rows) / 2, cv::Scalar(255, 255, 255), -1);
